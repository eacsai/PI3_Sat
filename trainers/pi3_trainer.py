...
class Pi3Trainer(BaseTrainer):
    ...
    def before_epoch(self, epoch):
        ...
        if hasattr(self.test_loader, 'dataset') and hasattr(self.test_loader.dataset, 'set_epoch'):
            self.test_loader.dataset.set_epoch(0, base_seed=self.cfg.train.base_seed) # 把数据集顺序打乱
    ...
    def calculate_loss(self, output, batch, mode='train', epoch=None):
        output, batch = output

        if mode == 'train':
            loss, details = self.train_loss(output, batch, epoch=epoch)
        else:
            loss, details = self.test_loss(output, batch, epoch=epoch)
