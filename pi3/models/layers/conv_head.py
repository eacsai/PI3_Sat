...
class ConvHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, image: torch.Tensor = None, patch_h: int = None, patch_w: int = None):
        ...
        return feat_low, feat_mid, feat_high # 吐出三层金字塔！
    
class InfiniDepthFusion(nn.Module):
    def __init__(self, dims=[128, 256, 512], target_dim=1024):
        """
        dims: 按分辨率从高到低排列。
              [128(输出层 1/1), 256(中间层 1/7), 512(投影层 1/14)]
        """
        super().__init__()
        # 融合 尺度1(High) -> 尺度2(Mid)
        self.linear1 = nn.Linear(dims[0], dims[1])
        self.g1 = nn.Parameter(torch.zeros(1, 1, dims[1])) # 门控 g1
        self.ffn1 = nn.Sequential(
            nn.Linear(dims[1], dims[1] * 4),
            nn.GELU(),
        ...
        self.g2 = nn.Parameter(torch.zeros(1, 1, dims[2])) # 门控 g2
        self.ffn2 = nn.Sequential(
            nn.Linear(dims[2], dims[2] * 4),
            nn.GELU(),
            nn.Linear(dims[2] * 4, dims[2])
        )

        # 映射给 D4RT 的维度
        self.out_proj = nn.Linear(dims[2], target_dim)

        # ==========================================================
        # 强制最后一层输出为 0，防止随机噪声在初期摧毁交叉注意力！
        # ==========================================================
        # nn.init.constant_(self.out_proj.weight, 0)
        # nn.init.constant_(self.out_proj.bias, 0)

    def forward(self, f_high, f_mid, f_low):
        # f_high: [B*N, Q, 128]
        # f_mid:  [B*N, Q, 256]
        # f_low:  [B*N, Q, 512]
        ...
        # 1. 高频(细粒度) -> 融入中频
        h1 = f_high
        h2_proj = self.linear1(h1)
        h2 = self.ffn1(f_mid + torch.sigmoid(self.g1) * h2_proj)

        # 2. 中频 -> 融入低频(强语义)
        h3_proj = self.linear2(h2)
        h3 = self.ffn2(f_low + torch.sigmoid(self.g2) * h3_proj)

        # 3. 输出给 D4RT
        return self.out_proj(h3)